...
def process_entry(entry):
    ...
    point.time(
        entry["info"].split()[1] + "T" + entry["info"].split()[2] + "Z",
        WritePrecision.NS,
    )
...
def main(args):
    ...
    write_api = client.write_api(
        write_options=WriteOptions(batch_size=1000, flush_interval=30_000)
    )
    ...
    for line in entries:
        try:
            ...
            sys.stdout.write(
                f"\rProcessed {counter} / {total_entries} successfully. Progress: {(counter / total_entries) * 100:.2f}%"
            )
            sys.stdout.flush()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="i2rp",
        description="Influx2 Results Processor:\nProcess and import Opsview Results Exporter data into InfluxDB 2.x.",
    )
    parser.add_argument(
        "--url", required=True, help="The URL of the InfluxDB instance."
    )
    ...
    parser.add_argument(
        "--bucket", required=True, help="The bucket name where data will be stored."
    )
    parser.add_argument(
        "--file",
        required=True,
        help="The path to the input file containing data entries.",
    )
